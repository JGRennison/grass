...
bool grass_app::CmdLineProc(wxChar ** argv, int argc) {
	...
	if (game_action) {
		bool ok = game_action();
		if (ok && trainlist) {
			main_gui::trainwin *trainlist = new main_gui::trainwin(w, panelset, main_gui::GetTrainwinColumnSet());
			trainlist->Show(true);
		}
		if (ok && textinput) {
			main_gui::cmd_input_win *cmdwin = new main_gui::cmd_input_win(w, panelset);
			cmdwin->Show(true);
		}
		return ok;
	} else {
